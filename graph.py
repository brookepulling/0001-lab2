...
class DirectedRoad():
    ...
    def __str__(self):
        ...
        """
        return (str(self.src_node) + ' -> ' + str(self.dest_node) + ' takes ' + str(self.travel_time) + ' minute(s) via ' + str(self.road_type) + ' road with traffic multiplier ' + str(self.traffic_multiplier))
...
class RoadMap():
    ...
    def insert_node(self, node):
        ...
        """
        if node in self.nodes:
            raise ValueError('node is already in roadmap')
        else: 
            self.nodes.add(node)
    ...
    def insert_road(self, road):
        ...
        if road.get_destination_node() in self.nodes and road.get_source_node() in self.nodes:
    ...
    def get_reachable_roads_from_node(self, node, restricted_roads):
        ...
        if node in self.nodes:
            ...
            for road in self.nodes_to_roads[node]:
                roads_out_node.append(road)
        final = []
        if node in self.nodes:
            for road in roads_out_node:
